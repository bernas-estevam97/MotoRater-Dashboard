    ...
    with tab4:
        ...
        if 'mapping_df' not in st.session_state:
            ...
            if len(selected_plot_groups) != 2:
                ...
                # FIX: Keep BOTH time columns for the dual-engine approach
                final_df['_time_'] = final_df['Timepoint_Weeks'].astype(str)       # Categorical for Stats Tables
                final_df['_time_cont_'] = final_df['Timepoint_Weeks'].astype(float) # Continuous for f(x) functions
                ...
                if len(selected_plot_timepoints) == 1:
                    with st.expander("📊 Cross-Sectional Stats (Click to Expand)", expanded=False):
                        ...
                        if len(g1_data) < 2 or len(g2_data) < 2:
                            st.error(f"🚨 Not enough data at Week {tp}. Group {selected_plot_groups[0]} has n={len(g1_data)}, Group {selected_plot_groups[1]} has n={len(g2_data)}.")
                        else:
                            base_model = smf.ols("_metric_ ~ C(_group_)", data=final_df).fit()
                            residuals = base_model.resid
                            
                            # --- 1. THE STRICT NORMALITY TEST ---
                            ...
                            try:
                                # USE THE NEW ROUTING LOGIC HERE
                ...
                # =====================================================================
                    ...
                    with st.expander("📊 Statistical Tables, Assumptions & f(x) Functions", expanded=False):
                        ...
                        if min_n < 3:
                            ...
                            # --- ENGINE 1: THE CATEGORICAL STATS (For Tables & Asterisks) ---
                            try:
                            ...
                            if use_parametric and is_balanced:
                                ...
                                try:
                                    ...
                                    for tp in selected_plot_timepoints:
                                        ...
                                        if len(g1) > 1 and len(g2) > 1:
                                            ...
                                            if p_val_found:
                                                rows.append({'Timepoint_Weeks': tp, 'Group A': selected_plot_groups[0], 'Group B': selected_plot_groups[1], 'p_unc': res[p_val_found].values[0], 'Cohen_d': res[eff_col].values[0] if eff_col else np.nan})
                                    if rows:
                                ...
                                try:
                                    ...
                                    for tp in selected_plot_timepoints:
                                        ...
                                        # Safely flatten the arrays to guarantee Python extracts a standalone scalar
                            ...
                            # --- ENGINE 2: THE CONTINUOUS MATH (For f(x) and Trendlines) ---
                            try:
                                groups_sorted = sorted(final_df['_group_'].unique())
                                ref_group, test_group = groups_sorted[0], groups_sorted[1]
            ...
            if 'final_df' in locals() and not final_df.empty:
                ...
                if plot_format == "Line Plot":
                    fig = px.line(summary_df, x='Timepoint_Weeks', y=plot_metric, color='Group', markers=True, error_y='SEM' if show_error_bars else None, title=title_text, color_discrete_map=color_map if 'color_map' in locals() else None)
                elif plot_format == "Bar Plot":
                    fig = px.bar(summary_df, x='Timepoint_Weeks', y=plot_metric, color='Group', barmode='group', error_y='SEM' if show_error_bars else None, title=title_text, color_discrete_map=color_map if 'color_map' in locals() else None)
                elif plot_format == "Box Plot":
                    fig = px.box(final_df, x='Timepoint_Weeks', y=plot_metric, color='Group', points="all" if show_points else False, title=title_text, color_discrete_map=color_map if 'color_map' in locals() else None)
                elif plot_format == "Violin Plot":
                    fig = px.violin(final_df, x='Timepoint_Weeks', y=plot_metric, color='Group', box=True, points="all" if show_points else False, title=title_text, color_discrete_map=color_map if 'color_map' in locals() else None)

                if plot_format == "Line Plot" and show_growth_curves and 'function_dict' in locals() and function_dict:
                    ...
                    for grp, equation in function_dict.items():
                        ...
                        line_color = color_map.get(grp, 'gray') if 'color_map' in locals() else 'gray'
                    ...
                    if p_col:
                        ...
                        for _, row in display_posthocs.iterrows():
                            ...
                            try:
                                ...
                                fig.add_annotation(x=tp, y=y_pos, text=star, showarrow=False, font=dict(size=14 if star == "ns" else 22, color=annotation_color if 'annotation_color' in locals() else "black", family="Arial"))
                            except Exception as e: pass
                                
                        fig.update_layout(yaxis=dict(range=[y_min_overall - (offset * 0.5), highest_drawn_y + (offset * 1.5)]))

                fig.update_layout(margin=dict(t=30))
                # Add this right before st.plotly_chart if you want to disable grid lines
                fig.update_xaxes(showgrid=False)
                fig.update_yaxes(showgrid=True)

                # Update the final Streamlit call to this:
            ...
            if st.button("Generate Radar Plot", type="primary", key="generate_radar_btn"):
                if len(radar_metrics) < 3:
                    ...
                    if merged_r.empty:
                        ...
                        # Add this right before st.plotly_chart if you want to disable grid lines
                        # fig_radar.update_xaxes(showgrid=True)
                        # fig_radar.update_yaxes(showgrid=True)
