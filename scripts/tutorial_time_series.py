...
2. Choose your input mode: **Upload Files** (drag-and-drop) or **Local Folder** (if running locally). 
3. Select or upload your MotoRater `.xlsx`, `.xls` or `.parquet` / `.h5` files.
...
* **Select a File & Sheet:** Choose the specific data file and worksheet (e.g., `Kinematics`) you want to analyze.
...
* **Box Plot:** Drops the timeline entirely to show the statistical distribution (median, quartiles, and outliers) of your selected measurements. Highly recommended when comparing multiple files!
...
* **Smart Filtering:** If you are analyzing a "filtered" Kinematics file, the dashboard automatically removes the bottom rows of metadata to prevent the app from crashing and ensures your data types are clean.
        """)