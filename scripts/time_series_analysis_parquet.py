...
st.markdown("Visualize and compare time-series data from MotoRater Parquet and HDF5 files.")

# --- Session State ---
if 'parquet_folder_path' not in st.session_state:
    st.session_state.parquet_folder_path = ''
if 'parquet_uploader_key' not in st.session_state:
    st.session_state.parquet_uploader_key = 0
...
# Map of item_name -> (type, reference)
# type: 'dir' (local directory of sheet parquets), 'local_file' (path to .parquet/.h5), 'uploaded' (UploadedFile)
items_map = {}
...
if source_mode == "📂 Local Folder":
    ...
    with col1:
        if st.button("📂", help="Open native folder picker"):
            selected = safe_select_folder(title="Select MotoRater Parquet/HDF5 Folder")
            if selected:
                st.session_state.parquet_folder_path = selected

    with col2:
        folder_path = st.text_input(
            "Path", 
            value=st.session_state.parquet_folder_path, 
            label_visibility="collapsed",
            placeholder="C:/path/to/folder"
        )

    if folder_path != st.session_state.parquet_folder_path:
        st.session_state.parquet_folder_path = folder_path

    if folder_path and os.path.isdir(folder_path):
        for entry in os.listdir(folder_path):
            full_p = os.path.join(folder_path, entry)
            if os.path.isdir(full_p):
                # Check if subfolder has valid data files
                if any(fname.endswith(valid_exts) for fname in os.listdir(full_p)):
                    items_map[entry] = ('dir', full_p)
            elif entry.endswith(valid_exts):
                items_map[entry] = ('local_file', full_p)
    elif folder_path:
    ...
    uploaded_files = st.sidebar.file_uploader(
        ...
        key=f"parquet_uploader_{st.session_state.parquet_uploader_key}"
    )
    if uploaded_files:
        for uf in uploaded_files:
            items_map[uf.name] = ('uploaded', uf)

files = list(items_map.keys())


# --- Helpers: Multi-Format Loading ---
@st.cache_data(show_spinner=False, max_entries=50, ttl=1800)
def get_data_sheets_for_item(item_name: str, item_type: str, item_ref):
    if item_type == 'dir':
        sheets = []
        for f in os.listdir(item_ref):
            for ext in valid_exts:
                if f.endswith(ext):
                    sheets.append(f.replace(ext, ''))
        return sorted(list(set(sheets)))
    elif item_name.endswith('.parquet'):
        return ["Data (Parquet)"]
    elif item_name.endswith(('.h5', '.hdf5')):
        try:
            if item_type == 'local_file':
                with pd.HDFStore(item_ref, mode='r') as store:
                    return [k.lstrip('/') for k in store.keys()]
            else:
                # UploadedFile
                file_bytes = item_ref.getvalue()
...
@st.cache_data(show_spinner=False, max_entries=50, ttl=1800)
def load_data_for_item(item_name: str, item_type: str, item_ref, sheet_name: str):
    try:
        df = None
        if item_type == 'dir':
            file_path = None
            ext_used = None
            for ext in valid_exts:
                temp_p = os.path.join(item_ref, f"{sheet_name}{ext}")
            ...
            if item_type == 'local_file':
                df = pd.read_parquet(item_ref, engine='pyarrow')
            else:
                df = pd.read_parquet(io.BytesIO(item_ref.getvalue()), engine='pyarrow')
        elif item_name.endswith(('.h5', '.hdf5')):
            if item_type == 'local_file':
                df = pd.read_hdf(item_ref, key=sheet_name)
            else:
                file_bytes = item_ref.getvalue()
                with tempfile.NamedTemporaryFile(delete=False, suffix='.h5') as tmp:
                    tmp.write(file_bytes)
        ...
        # Filter logic
        if df is not None and "filtered" in item_name.lower() and ("kinematics" in item_name.lower() or sheet_name == "Kinematics"):
            if len(df) > 6:
                df = df.iloc[:-6]
        return df
    except Exception:
...
# --- Main Logic ---
if len(files) > 0:
    st.sidebar.success(f"Loaded {len(files)} data item(s).")
    ...
    # ==========================================
    if not compare_mode:
        selected_file = st.sidebar.selectbox("Select Data:", files)
        itype, iref = items_map[selected_file]
        sheet_names = get_data_sheets_for_item(selected_file, itype, iref)

        if sheet_names:
            selected_sheet = st.sidebar.selectbox("Select Sheet:", sheet_names) if len(sheet_names) > 1 else sheet_names[0]
            df = load_data_for_item(selected_file, itype, iref, selected_sheet)
    ...
    # ==========================================
        ...
        if len(selected_files) < 2:
            st.info("👈 Please select at least two items from the sidebar to compare.")
        else:
            ...
            def get_sheets_with_ctx(fname):
                ...
                itype, iref = items_map[fname]
                return get_data_sheets_for_item(fname, itype, iref)
            ...
            if not common_sheets:
                st.error("No common sheets found among the selected items.")
            else:
                ...
                def fetch_data_with_ctx(fname):
                    add_script_run_ctx(threading.current_thread(), ctx)
                    itype, iref = items_map[fname]
                    return fname, load_data_for_item(fname, itype, iref, common_sheet)
